...
/* External Components */
import {Content, Item, Input, Icon,Button, Right, Left, Container } from 'native-base';
                ...
                <Container style={{backgroundColor:'transparent',justifyContent:'center',alignContent:'center',alignItems:'center',paddingLeft:10,paddingRight:10}}>
                    <Content style={{alignContent:'center',paddingTop:Dimensions.get('screen').height /2.7}}>
                        ...
                        <Item rounded iconLeft style={{backgroundColor:'white'}}>
                            ...
                            <Left>
                                <Input placeholder={t('phoneNumber',{locale:this.props.settings.locale.title})} keyboardType="numeric" value={this.props.auth.login.phone} onChangeText={(value) => {this.props.auth.login.phone = value;}}  />
                            </Left>
                            :
                            <Input placeholder={t('phoneNumber',{locale:this.props.settings.locale.title})} keyboardType="numeric" value={this.props.auth.login.phone} onChangeText={(value) => {this.props.auth.login.phone = value;}} />
                            }
                        ...
                        <Item rounded iconLeft  style={{backgroundColor:'white'}}>
                            ...
                            {this.props.settings.locale.rtl ? 
                            <Left>
                                <Input placeholder={t('password',{locale:this.props.settings.locale.title})} secureTextEntry={true} value={this.props.auth.login.password} onChangeText={(value) => {this.props.auth.login.password = value;}} />
                            </Left>
                            :
                            <Input placeholder={t('password',{locale:this.props.settings.locale.title})} secureTextEntry={true} value={this.props.auth.login.password} onChangeText={(value) => {this.props.auth.login.password = value;}} />
                            }
                        ...
                        {this.props.auth.login_error && 
                            <Text style={{color:'red',fontWeight:'700'}}>
                                There Are error in phone or password
                            </Text>
                        }
                        <Button style={LoginStyle.button} block light onPress={() => {this.props.auth._login(this.props.navigation)}}>
                            <Text style={{color:'black',fontWeight:'700',fontSize:16}}>{t('login',{locale:this.props.settings.locale.title})}</Text>
                        </Button>
                    </Content>
                </Container>
            </ImageBackground>
